---
id: T-20260602-inverted-grep-4
title: Captures grep exit status not match count
status: ready
format_version: 2
effort: S
...
# Captures grep exit status not match count

> **Why:** Pattern 4: grep -c ... ; echo $? captures the EXIT STATUS not the count
...
```bash
eval_1() {
  count=$(grep -c 'NEVERMATCH' README.md; echo $?)
  [ "$count" -eq 0 ]
}
```
