...
#
# Usage:
#   bash tools/setup-taskspec-signing-key.sh                 # default location
#   bash tools/setup-taskspec-signing-key.sh --path PATH     # explicit file
#   bash tools/setup-taskspec-signing-key.sh --print-only    # print key+keyid, write nothing
#   bash tools/setup-taskspec-signing-key.sh --force         # overwrite an existing key
#   bash tools/setup-taskspec-signing-key.sh --version
#
# Exit codes:
...
# _lib.sh lives in ../src/lib relative to configs/.
# shellcheck source=../src/lib/_lib.sh
source "$SCRIPT_DIR/../src/lib/_lib.sh"
...
echo "(full crypto trust). Verify with: taskspec validate <spec>"
exit 0
