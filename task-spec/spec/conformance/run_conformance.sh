...
# --- Paths ---
# CONF_DIR = spec/conformance; the engine repo root is two levels up.
CONF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$(cd "$CONF_DIR/../.." && pwd)"
VALIDATOR="$SKILL_DIR/src/gate/validate-task-spec.sh"
...
# section. Ported from src/gate/run-task-spec.sh (same G7 heredoc-safety logic)
