...
depends_on: []
touches_paths:
  - spec/conformance/_workdir/c001.log
...
```bash
eval_1() {
  test -f spec/conformance/_workdir/c001.log
}

eval_2() {
  test "$(grep -c 'claim_won' spec/conformance/_workdir/c001.log)" -eq 1
}

eval_3() {
  test "$(grep -c 'claim_aborted_status_not_ready' spec/conformance/_workdir/c001.log)" -ge 1
}
```
...
agent_contract:
  ...
  sandbox_type: host
  output_artifacts:
    - path: spec/conformance/_workdir/c001.log
      type: log
  mcp_dependencies: []
  emit:
    - pass
...
Delete `spec/conformance/_workdir/c001.log` and re-run.
