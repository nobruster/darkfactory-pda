...
1. Clone the task-spec repo (https://github.com/luanmorenommaciel/task-spec —
   or fetch the released archive) at the SHA you want to vendor.
2. Copy the entire `spec/conformance/` directory into your repo's
   `tests/conformance/task-spec/` (or a path of your choice).
3. Create a `VENDORED_FROM` file containing the source URL and the SHA you
   vendored from. Example:

   ```text
   https://github.com/luanmorenommaciel/task-spec/tree/<sha>/spec/conformance
   sha: <sha>
...
- `../../docs/concepts/agent-contract.md` — the contract itself
- `../../docs/concepts/signed-off.md` — the autonomy gate
