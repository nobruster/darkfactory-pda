# Reference

- [CLI and exit codes](cli.md)
- [Portable contracts](contracts.md)
- [TaskRevision/v1](task-revision.md)
- [TaskGraphView/v1](task-graph-view.md)
- [TaskMesh contracts and runtime model](taskmesh-contracts.md)
- [Handoff, receipt, and acceptance contracts](acceptance-contracts.md)
- [Compatibility policy](compatibility-policy.md)
- [Converge donor map](converge-donor-map.md)
- [Task-Spec format v3](../../spec/task-spec-v3.md)
- [JSON Schemas](../../spec/schemas/README.md)
- [Agent contract](../concepts/agent-contract.md)
- [Quick reference](../quick-reference.md)

For tools, `taskspec agent-context` is the authoritative machine-readable
command/mutation/token/exit-code inventory.
