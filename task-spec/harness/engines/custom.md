...
When v2.2 ships, the dispatcher will read `dispatch_recipe: harness/engines/my-engine.md` from the frontmatter and invoke the recipe automatically.
...
| No path back to a supported engine | Document the bespoke recipe under `harness/engines/<engine>.md` so the next operator can re-dispatch |
