# Lesson — Pass 2 (Structure): CI pipeline grounding (decoy-section fixture)

> Negative fixture: the real "## Why this pass exists" section is absent and a
> decoy "## Notes on why this pass exists" stands in its place. Section
> matching is exact-name only, so the gate must FAIL on the missing section —
> a substring match would have let the decoy satisfy it.
