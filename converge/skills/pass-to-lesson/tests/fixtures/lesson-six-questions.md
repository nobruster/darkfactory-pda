# Lesson — Pass 2 (Structure): CI pipeline grounding (six-questions fixture)

> Negative fixture: 6 check-yourself questions (3–5 required). Everything else
> is valid, so the gate must FAIL on the question count.
...
4. Why did bind-mounting the socket lose? (→ see "Decisions and roads not taken")
5. Why did static cloud keys lose? (→ see "Decisions and roads not taken")
6. What does the runner image pin mean for re-verification? (→ see "What to watch")
