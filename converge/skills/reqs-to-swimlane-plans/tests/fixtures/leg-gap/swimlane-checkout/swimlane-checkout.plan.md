...
flowchart LR
  ...
  V --> P["leg-03-stripe · charge + order"]
...
2. leg-01 confirms the cart is purchasable; leg-03 charges and writes the order.
...
| **leg-03-stripe** | charge and write the order record | [swimlane-checkout-leg-03-stripe.md](swimlane-checkout-leg-03-stripe.md) |
...
```
cart.*  ->  leg-01  ->  leg-03  ->  order.*
```

## Build order

1. **leg-01** — validate first; nothing charges an invalid cart.
2. **leg-03** — charge + order once validation holds.
