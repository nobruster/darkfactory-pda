# Tech-Spec — Fixture: requirement bullets without R-n/W-n ids must warn
...
- (must) A failed run is detected within 2 minutes of failure, measured
  failure-timestamp -> detection-timestamp, p95 <= 2 minutes.
- (must) Transient failures are retried at most 2 times; a run that fails
  3 times escalates within 5 minutes to the on-call owner.
- (should) Weekly hand-triage time drops to under 1 hour.
...
- id: GAP-001
  ...
  blocks: "the retry requirement's acceptance"
  owner: "Platform Lead"
