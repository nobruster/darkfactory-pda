# Tech-Spec — Fixture: CI pipeline recovery lane ('sparkline' must NOT warn)

## Problem restated

Dana Sparks, the platform sponsor, put it plainly: broken CI runs sit
unnoticed for 45 minutes (measured) before anyone reacts, and each week
roughly 6 hours (estimated) of engineer time goes to re-running and
bisecting failed pipelines by hand. The incident review sparked this
brief. Solved means failures are detected, triaged, and either
auto-retried or escalated within a known time bound — with sparkline
trends on the weekly review so the improvement is visible.
...
- id: GAP-001
  ...
  owner: "Platform Lead"
  resolution: "RESOLVED 2026-07-19: 20 retries/day ceiling"
```
