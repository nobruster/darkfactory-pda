#!/usr/bin/env python3
"""Attach golden-match to Type 03 modern observations. Does not edit the referee."""
...
CONTRACT = REPO_ROOT / "contracts" / "types" / "03-payment-slip-settlement" / "main"
...
HAPPY_BATCH = "B202607230000201"
DF_SOURCE_003_BATCH = "B202607230000205"
...
def _landing_records(batch_id: str) -> list[dict[str, Any]]:
    ...
    for path in files:
        ...
        for row in table.to_pylist():
            converted = dict(row)
            for key in ("face_amount_brl", "discount_brl", "fee_brl", "net_amount_brl"):
...
def _gold_row(batch_id: str) -> dict[str, Any] | None:
    ...
    try:
        result = con.execute(
            "select * from gold.gold_payment_slip_settlement_reconciliation "
            "where batch_id = ?",
            [batch_id],
        )
...
def attach_happy() -> golden_match.Comparison:
    ...
    comparison = golden_match.Comparison(HAPPY_BATCH, "03", "accepted")
...
def attach_df_source_003() -> golden_match.Comparison:
    modern = _parser_run(DF_SOURCE_003_BATCH)
    contract = _load_yaml(CONTRACT / "expected-df-source-003-finding.yaml")
    comparison = golden_match.Comparison(DF_SOURCE_003_BATCH, "03", "source-defect")
    differences, checks = golden_match.compare_rejection(
        ...
        batch_id=DF_SOURCE_003_BATCH,
    )
    comparison.differences.extend(differences)
    comparison.checks.update(checks)
    comparison.checks["gold_absent"] = _gold_row(DF_SOURCE_003_BATCH) is None
    landing_dir = LANDING / DF_SOURCE_003_BATCH
...
def main() -> int:
    happy = attach_happy()
    lie = attach_df_source_003()
    _write(HAPPY_BATCH, "golden-match.json", happy.as_dict())
    _write(DF_SOURCE_003_BATCH, "golden-match.json", lie.as_dict())
    _write(
        HAPPY_BATCH,
        "difference-adjudication.json",
        {
            "happy_resolved": happy.resolved,
            "df_source_003_unexplained": len(lie.unexplained),
        },
    )
    print(
        json.dumps(
            {"valid-minimal": happy.as_dict(), "df-source-003": lie.as_dict()},
            indent=2,
        )
    )
