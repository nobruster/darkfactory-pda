"""Compose parse -> schema -> writer for one Type 03 batch. First write is landing Parquet."""
...
def _load_parser():  # type: ignore[no-untyped-def]
    ...
    spec = importlib.util.spec_from_file_location("nwp_t03_parser", path)
    if spec is None or spec.loader is None:
        raise ImportError("parser.py")
    module = importlib.util.module_from_spec(spec)
    sys.modules["nwp_t03_parser"] = module
...
def _source_filename(payload: bytes, fallback: str) -> str:
    ...
    if file_date and batch_id:
        return f"NW_PAYMENT_SLIP_{file_date}_{batch_id}.rem"
    return fallback
...
def process(raw_path: Path, *, landing_root: Path) -> BatchOutcome:
    ...
    parsed = parser.parse_payment_slip_settlement(payload, filename=source_filename)
    ...
    parquet_name = source_filename.replace(".rem", ".parquet")
    ...
    published = publish(
        ...
        manifest={
            "batch_id": batch_id,
            "computed_logical_count": len(records),
            "computed_net_amount": controls["computed_net_amount"],
            "contract_code": "PAYSLIPSET03",
            "currency": "BRL",
            "declared_logical_count": controls["declared_logical_count"],
