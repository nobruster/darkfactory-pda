...
my-custom-engine \
  ...
  --post-eval "taskspec gate $SPEC"
```

When v2.2 ships, the dispatcher will read `dispatch_recipe: adapters/engines/my-engine.md` from the frontmatter and invoke the recipe automatically.
...
| No path back to a supported engine | Document the bespoke recipe under `adapters/engines/<engine>.md` so the next operator can re-dispatch |
...
- [../../docs/concepts/agent-contract.md](../../docs/concepts/agent-contract.md) — the contract every engine must honor
- [../../docs/concepts/signed-off.md](../../docs/concepts/signed-off.md) — autonomy contract
