...
- Every new T-*.md is committed immediately by `src/author/generate-task-spec.sh`
...
- Status changes happen via `src/backlog/transition-status.sh` (atomic + logged)
...
**`_state.yaml` is REBUILDABLE.** Run `src/backlog/rebuild-state.sh` and it scans
...
This is `src/backlog/transition-status.sh` in essence. All 8 steps OR all 8 rollback.
...
```bash
# taskspec backup — run via cron daily at 03:00 local
...
- [task-spec-v1.md](../../spec/task-spec-v3.md) — the format that produces files in this backlog
