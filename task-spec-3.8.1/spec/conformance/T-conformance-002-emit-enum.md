...
depends_on: []
touches_paths:
  - spec/conformance/_workdir/c002_metrics.jsonl
...
```bash
eval_1() {
  test -f spec/conformance/_workdir/c002_metrics.jsonl
}

eval_2() {
  jq -r '.outcome' spec/conformance/_workdir/c002_metrics.jsonl \
...
eval_4() {
  test "$(jq -r '.reason' spec/conformance/_workdir/c002_metrics.jsonl | tail -1)" != "null"
}
```
...
agent_contract:
  ...
  sandbox_type: host
  output_artifacts:
    - path: spec/conformance/_workdir/c002_metrics.jsonl
      type: log
  mcp_dependencies: []
  emit:
    - pass
...
Truncate `spec/conformance/_workdir/c002_metrics.jsonl` and re-run.
