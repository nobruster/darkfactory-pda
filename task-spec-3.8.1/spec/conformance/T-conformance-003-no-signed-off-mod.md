...
depends_on: []
touches_paths:
  - spec/conformance/_workdir/c003_envelope_before.txt
  - spec/conformance/_workdir/c003_envelope_after.txt
...
```bash
eval_1() {
  test -f spec/conformance/_workdir/c003_envelope_before.txt
}

eval_2() {
  test -f spec/conformance/_workdir/c003_envelope_after.txt
}

eval_3() {
  diff -q spec/conformance/_workdir/c003_envelope_before.txt \
          spec/conformance/_workdir/c003_envelope_after.txt
}
```
...
agent_contract:
  ...
  sandbox_type: host
  output_artifacts:
    - path: spec/conformance/_workdir/c003_envelope_before.txt
      type: snapshot
    - path: spec/conformance/_workdir/c003_envelope_after.txt
      type: snapshot
  mcp_dependencies: []
  emit:
    - pass
...
Delete `spec/conformance/_workdir/c003_envelope_*.txt` and re-run.
