---
id: T-20260602-inverted-grep-2
title: Inverted grep -c with stderr suppression
status: ready
format_version: 2
effort: S
...
# Inverted grep -c with stderr suppression

> **Why:** Pattern 2: same bug with 2>/dev/null suppression
...
```bash
eval_1() {
  count=$(grep -c 'NEVERMATCH' README.md 2>/dev/null || echo 0)
  [ "$count" -eq 0 ]
}
```
