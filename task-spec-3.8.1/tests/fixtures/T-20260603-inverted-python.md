---
id: T-20260603-inverted-python
title: Inverted python -c count with echo 0 fallback
status: ready
format_version: 2
effort: S
...
# Inverted python -c count with echo 0 fallback

> **Why:** python is not grep -c or wc, so the per-command fast-paths don't fire; the umbrella rule must catch any $(cmd) || echo 0 before a numeric compare
...
```bash
eval_1() {
  count=$(python3 -c 'print(0)' 2>/dev/null || echo 0)
  [ "$count" -eq 0 ]
}
```
