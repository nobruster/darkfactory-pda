...
#
# Asserts:
#   (a) `bash -n` (syntax check) passes for every .sh under src/ using the
#       bash this test runs under.
#   (b) The four core-path scripts (gate/validate-task-spec.sh,
...
# ---------------------------------------------------------------------------
# (a) bash -n syntax check for every .sh under src/ (all verb subdirs)
# ---------------------------------------------------------------------------
echo "=== (a) bash -n syntax check for every src/*/*.sh ==="
shopt -s nullglob
...
CORE_SCRIPTS=(gate/validate-task-spec.sh gate/safe-to-delegate.sh gate/run-task-spec.sh lib/_lib.sh)
...
CONF_DIR="$REPO_ROOT/spec/conformance"
shopt -s nullglob
